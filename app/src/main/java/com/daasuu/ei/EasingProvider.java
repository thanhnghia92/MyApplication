...
class EasingProvider {
    ...
    public static float get(@NonNull Ease ease, float elapsedTimeRate) {
        switch (ease) {
            ...
            case SWING:
                return getSwing(elapsedTimeRate);
            default:
                return elapsedTimeRate;
    ...
    private static float getSwing(float elapsedTimeRate){
        return (float) (Math.pow(2, (-10 * elapsedTimeRate)) * Math.sin(((2* Math.PI) * (elapsedTimeRate - (0.3f/4)))/0.3f) + 1);
    }
}
